...
const Interview = () => {
    ...
    const loadHistory = async (companyId) => {
        try {
            const s = await apiClient.getInterviewSessions(companyId);
    ...
    const handleHelpMe = async () => {
        ...
        setIsCoaching(true);
        try {
            const coaching = await apiClient.getInterviewHelp(activeCompany.company_id, session.session_id);
    ...
    const handleResetCompany = async () => {
        try {
            await apiClient.clearInterviewSessions(activeCompany.company_id);
            setSessionsHistory([]);
            setSession(null);
            setScoreData(null);
                ...
                <div className="space-y-2 mb-6 max-h-[30%] overflow-y-auto custom-scrollbar">
                    {targets.map(t => (
                        <button
                            ...
                            className={`w-full text-left p-3 rounded-xl border transition ${activeCompany?.company_id === t.company_id ? 'bg-[#6C63FF]/20 border-[#6C63FF] text-white' : 'bg-zinc-950/50 border-zinc-800 text-zinc-400 hover:border-zinc-600'
                                }`}
                        >
                        ...
                        <form onSubmit={handleSendMessage} className="p-4 bg-zinc-950 border-t border-zinc-800 shrink-0">
                            <div className="relative">
                                <input
                                    ...
                                    disabled={isTyping || isCoaching || session?.session_id === 'loading'}
                                ...
                                <button
                                    type="submit"
                                    disabled={!inputMsg.trim() || isTyping || session?.session_id === 'loading'}
