         ...
         <div className="bg-zinc-950 border border-amber-500/30 shadow-2xl shadow-black rounded-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90%]">
            ...
            <div className="p-6 space-y-6 overflow-y-auto custom-scrollbar">
               ...
               <div>
                  ...
                  <p className="text-sm text-zinc-300 p-3 bg-zinc-900 rounded-xl border border-zinc-800 whitespace-pre-wrap">
                     {renderText(data.question_intent)}
                  </p>
               </div>
               ...
               <div>
                  ...
                  <div className="text-sm text-zinc-300 p-3 bg-zinc-900 rounded-xl border border-zinc-800 whitespace-pre-wrap">
                     {renderText(data.ideal_answer_structure)}
                  </div>
               </div>
               ...
               <div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                     {Array.isArray(data.keywords_to_mention) ? data.keywords_to_mention.map((kw, i) => (
                        <span key={i} className="px-3 py-1 bg-zinc-800 text-amber-300 text-xs font-mono rounded-full border border-amber-500/20 shadow-sm">
                           {typeof kw === 'object' ? JSON.stringify(kw) : String(kw)}
                        </span>
                     )) : (
                        <span className="text-zinc-500 text-xs">No keywords extracted.</span>
                     )}
                  </div>
               </div>
               ...
               <div className="relative">
                  ...
                  <div className="p-4 border border-zinc-700/50 rounded-xl relative">
                     ...
                     <p className="text-sm font-medium text-white italic whitespace-pre-wrap">"{renderText(data.coaching_tip)}"</p>
                  </div>
               </div>
